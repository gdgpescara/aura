    ...
    <v-container class="pa-0 mb-3">
        ...
        <v-layout wrap align-center justify-center row fill-height class="mt-0 mb-0" >
           <v-flex xs12 md12 lg12 class="pa-2 mb-0">
               <p class="google-font mb-0" style="font-size:170%;color:#0277bd">{{ $t('Events.Past') }}</p>
               <p class="google-font mt-0 mb-0" style="font-size:110%;color:#616161 ">
                   {{ $t('Events.Reversal') }}
               </p>     
               <p class="google-font" style="font-size:110%;color:#616161 ">{{ $t('Events.Recent') }} <a :href="chapterDetails.ChapterMeetupLink" target="_blank" style="text-decoration:none;color:#0277bd">{{ $t('Events.Click') }}</a></p>
        ...
        <v-layout wrap row >
            ...
            <v-flex xs12 v-if="notFoundPastEventFlag==true">
                <p class="google-font px-2" style="font-size:140%"><v-icon >highlight_off</v-icon> {{ $t('Events.PastNotFound') }}</p>
            </v-flex>
...
export default {
    ...
    created(){
        fetch('https://cors-anywhere.herokuapp.com/https://api.meetup.com/'+MeetupAPI.urlname+'/events?desc=true&photo-host=public&page=6&status=past&key='+MeetupAPI.apiKey).then(data=>data.json()).then(res=>{
            if(res.length>0){
